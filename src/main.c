...
{
	Writer screen = Writer(Tonight.Std.Console.Output);
	Writer write = Writer(Tonight.Std.File.Output);
	Scanner read = Scanner(Tonight.Std.File.Input);
	Painter paint = Painter(Tonight.Resources.Color);
	Timer now = Timer(Tonight.Std.TimeNow);
	...
	try
	{
		using(queue $as new(List.class) $with delete)
		{
			...
			while(true)
			{
				...
				try
				{
					...
					using(list $as new(List.class) $with delete)
					{
						file log = $Empty(file);
						file f = $Empty(file);
						int i = $Empty(int);
